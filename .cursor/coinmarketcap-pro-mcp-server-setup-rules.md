# CoinMarketCap Pro MCP Server Implementation Guide

You are working on implementing the CoinMarketCap Pro MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: CoinMarketCap Pro
- **Documentation**: [None](None)
- **Website**: [https://pro-api.coinmarketcap.com](https://pro-api.coinmarketcap.com)
- **Authentication**: Required - API key via `COINMARKETCAP_PRO_API_KEY` environment variable
...
When a client provides their own CoinMarketCap Pro API key:
...
Flow:
  1. Client connects with their CoinMarketCap Pro API key
  2. MCP server uses client's API key to call CoinMarketCap Pro API
  3. No payment required
  4. Client pays CoinMarketCap Pro directly (not the MCP server)
```

**Use Case**: Clients who already have a CoinMarketCap Pro subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL CoinMarketCap Pro API key to call the API
  5. Client pays the MCP server (not CoinMarketCap Pro)
```

**Use Case**: Pay-per-use clients without their own CoinMarketCap Pro subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("COINMARKETCAP_PRO_API_KEY")
...
**Required**:
- `COINMARKETCAP_PRO_API_KEY`: Server's internal CoinMarketCap Pro API key (used when clients pay via 402)
...
COINMARKETCAP_PRO_API_KEY=your_coinmarketcap-pro_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_coinmarketcap_pro",
      "get_coinmarketcap_pro_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["coinmarketcap pro", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the CoinMarketCap Pro API with retry support. Example pattern:
...
def call_coinmarketcap_pro_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call CoinMarketCap Pro API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"CoinMarketCap Pro API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_coinmarketcap_pro(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search CoinMarketCap Pro for [specific data type].
    ...
    try:
        # The call_coinmarketcap_pro_api function already has retry logic
        results = call_coinmarketcap_pro_api(
...
```python
@mcp.tool()
async def get_coinmarketcap_pro_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_coinmarketcap_pro_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in CoinMarketCap Pro.
...
- **Log retry attempts** for debugging:
  ...
  def call_coinmarketcap_pro_api(...):
      # Implementation
  ```
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_coinmarketcap_pro(query="example", limit=5)
   ```
...
### 9. Common CoinMarketCap Pro Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on CoinMarketCap Pro capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the CoinMarketCap Pro documentation that you should implement:
...
- Check the CoinMarketCap Pro API documentation: None
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make CoinMarketCap Pro's capabilities easily accessible to AI agents through standardized MCP tools. 
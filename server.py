#!/usr/bin/env python3
"""
CoinMarketCap Pro MCP Server - FastMCP with D402 Transport Wrapper
...
- COINMARKETCAP_PRO_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('coinmarketcap-pro_mcp')
...
API_KEY = os.getenv("COINMARKETCAP_PRO_API_KEY")
if not API_KEY:
    logger.warning(f"⚠️  COINMARKETCAP_PRO_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"CoinMarketCap Pro MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("CoinMarketCap Pro MCP Server", host="0.0.0.0")
...
# - COINMARKETCAP_PRO_API_KEY: Server's internal API key for payment mode
#
...
async def coinmarketcap_id_map_wqdt(
    context: Context,
    ...
    Example Usage:
        await coinmarketcap_id_map_wqdt()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_wqdt: {e}")
...
async def quotes_historical_tn3t(
    context: Context,
    ...
    try:
        ...
        logger.error(f"Error in quotes_historical_tn3t: {e}")
...
async def quotes_latest_unz1(
    ...
    try:
        ...
        logger.error(f"Error in quotes_latest_unz1: {e}")
...
async def coinmarketcap_id_map_cxma(
    context: Context,
    start: int = 1,
    limit: Optional[int] = None,
    sort: str = "id",
    ...
    Example Usage:
        await coinmarketcap_id_map_cxma()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_cxma: {e}")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="coinmarketcap-pro-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "coinmarketcap-pro-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting CoinMarketCap Pro MCP Server")
    logger.info("="*80)
